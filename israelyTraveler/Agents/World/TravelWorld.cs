...
{    
    public class TravelWorld:ICloneable
    {
        public override string ToString()
        {
            string res="";
            res += "Water Places:";
            res += StringList(_waterPlaces.ToList());
            res += Environment.NewLine;

            res += "Fires:";
            res += StringList(_fireWays.ToList());
            res += Environment.NewLine;
            return res;
        }

        private string StringList(IList list)
        {
            string res = "";
        ...
        Dictionary<BaseTraveler, int> _locations = new Dictionary<BaseTraveler, int>(2);

        public IEnumerable<int> GetWaterPlaces() { return _waterPlaces; }
        public IEnumerable<TravelEdge> GetFireWays() { return _fireWays; }
        public IDictionary<BaseTraveler, int> GetPlayersLocations() { return _locations; }
